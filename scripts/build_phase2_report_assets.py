#!/usr/bin/env python3
"""Build publication-ready Phase 2 tables and figures from committed artifacts."""
...
def build_figures(tables: dict[str, pd.DataFrame]) -> None:
    ...
    add_header(fig, "Phase 2 evidence matrix", "Core role-decomposition metrics across context length and needle position.")
    fig.subplots_adjust(top=0.82)
    ax.set_xlabel("Context length / needle position")
    ax.set_ylabel("")
...
<head>
  <meta charset="utf-8">
  <title>Phase 2 Retrieval Circuit Report Assets</title>
  <style>
...
<body>
  <h1>Phase 2 Retrieval Circuit Report Assets</h1>
...
"""
    (OUT_DIR / "phase2_report_preview.html").write_text(html_doc)


def write_readme() -> None:
    text = """# Phase 2 Report Assets
...
Open `phase2_report_preview.html` for a quick visual review.
